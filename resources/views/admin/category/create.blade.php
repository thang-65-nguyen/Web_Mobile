...
@section('content')
    <div class="px-4">
        <div class="py-8">
            <div>
                <h2 class="text-2xl font-semibold leading-tight">Create Category</h2>
            </div>
            <div class="my-4">
                <form method="POST" action="{{ route('category.store') }}">
                    ...
                    <div class="mb-4">
                        ...
                        <textarea id="description" name="description" rows="4" class=" outline-none p-2 mt-1 block w-full rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm @error('description') border-red-500 @enderror">{{ old('description') }}</textarea>
                        @error('description')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
