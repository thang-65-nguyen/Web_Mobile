...
@section('content')
    <div class="px-4">
        <div class="py-8">
            <div>
                <h2 class="text-2xl font-semibold leading-tight">Danh sách banner</h2>
            </div>
                    ...
                    <table class="min-w-full leading-normal">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                    <p class="text-gray-900 whitespace-nowrap">{{ $banner->title }}</p>
                                </td>
                                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                    <img src="{{ asset('storage/' . $banner->image) }}" alt="{{ $banner->title }}" class="w-28">
                                </td>
                                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                    <p class="text-gray-900 ">{{ $banner->description }}</p>
                                </td>
                                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                    <p class="text-gray-900 whitespace-nowrap">{{ $banner->position }}</p>
                                </td>
                                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                    <p class="text-gray-900 whitespace-nowrap">{{ $banner->start_date }}</p>
                                </td>
                                ...
                                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                    <a href="{{ route('banner.edit', $banner->id) }}"  class="bg-indigo-600 text-white px-2 py-1 ml-2"><i class="fa-solid fa-pen"></i></a>
                                    <form action="{{ route('banner.destroy', $banner->id) }}" method="POST" class="inline-block ml-4">
                                        @csrf
                                        @method('DELETE')
